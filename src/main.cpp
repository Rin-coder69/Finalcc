...
inline constexpr int  frameWidth = 518;
inline constexpr int  frameHeight = 280;
int FrameSpacing = 20;
Texture HS;
int frameCount = 4;
int frame = 0;
...
int main(void)
{
    ...
    Sound fxWav = LoadSound("microchip.wav"); // Load WAV audio file
	PlaySound(fxWav);// Play WAV audio file

    HS = LoadTexture("resources/lamo2.png");
...
void DrawGame(void)
{
    ...
    if (!gameOver)
    {   
        Rectangle source = { frame * frameWidth, 0, (float)HS.width/frameCount, (float)HS.height};
        DrawTexturePro(HS, source, Rectangle{ floppy.position.x,floppy.position.y, (float)frameWidth/2, (float)frameHeight/2 }, Vector2{0,0}, 0.0f, WHITE);
